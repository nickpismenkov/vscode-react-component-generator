...
export class FileHelper {
    ...
    public static createTypeCss(componentDir: string, componentName: string): Observable<string> {
      const globalConfig: GlobalInterface = getConfig().get('global');
      const styleConfig: CSSInterface = getConfig().get('styleFile');
      const styleTemplate = getStyleSheetExtTemplate();
      ...
      let typeFilename = `${componentDir}/${compName}${styleConfig.suffix}.${styleTemplate.typeExt}`;
      if (styleConfig.create) {
        return this.createFile(typeFilename, '')
          .map(result => typeFilename);
      }
      else {
        return Observable.of('');
      }
...
export function getStyleSheetExtTemplate() {
  ...
  switch (configuredView) {
    case 'styled-components (.ts)':
      styleTemplate = { ext: 'ts', template: 'css-styled.template', typeExt: '' };
      break;
    case 'emotion (.js)':
      styleTemplate = { ext: 'ts', template: 'css-emotion.template', typeExt: '' };
      break;
    case 'sass (.sass)':
    ...
    case 'sass (.scss)':
      ...
      styleTemplate.typeExt = 'scss.d.ts';
      break;
    case 'less (.less)':
